          ...
          <div className="flex-1 overflow-hidden">
            ...
            <AnimatePresence mode="wait" initial={false}>
              ...
              >
                ...
                <p className="font-[family-name:var(--font-space)] text-sm text-[#0A0A0A]/55 mb-6">
                  {current.period} · {current.tag}
                </p>

                <div className="w-full h-px mb-8" style={{ background: "#D8D2C8" }} />

                <p className="font-[family-name:var(--font-space)] text-base text-[#0A0A0A]/75 leading-relaxed mb-10">
                  {current.summary}
                </p>
                      ...
                      {current.designProcess.map((phase, i) => (
                        <div key={i} className="flex gap-5">
                          ...
                          <div>
                            ...
                            <p className="font-[family-name:var(--font-space)] text-sm text-[#0A0A0A]/60 leading-relaxed">
                              {phase.description}
                            </p>
                          </div>
                        </div>
                    ...
                    <ul className="space-y-3 mt-4">
                      {current.finalDesign.map((point, i) => (
                        <li key={i} className="flex gap-3">
                          ...
                          <p className="font-[family-name:var(--font-space)] text-sm text-[#0A0A0A]/65 leading-relaxed">
                            {point}
                          </p>
                        </li>
                      ))}
                    ...
                    <div className="grid grid-cols-2 gap-4 mt-4">
                      {current.results.map((r, i) => (
                        <div key={i} className="p-4" style={{ border: "1px solid #D8D2C8" }}>
                          ...
                          <p className="font-[family-name:var(--font-space)] text-sm text-[#0A0A0A]/55 leading-relaxed">
                            {r.detail}
                          </p>
                        </div>
                      ))}
                ...
                {!current.context && !current.problem && !current.designProcess && (
                  <div className="py-10 text-center" style={{ borderTop: "1px solid #D8D2C8" }}>
                    <p className="font-[family-name:var(--font-space)] text-base text-[#0A0A0A]/35 italic">
...
function SectionLabel({ children }: { children: React.ReactNode }) {
  return (
    <p className="font-[family-name:var(--font-bebas)] text-base tracking-widest text-[#0A0A0A]/45 mb-0">
      {children}
    </p>
  );
}
...
function Section({ label, body }: { label: string; body: string }) {
  return (
    <div className="mb-10">
      ...
      <p className="font-[family-name:var(--font-space)] text-base text-[#0A0A0A]/65 leading-relaxed mt-3">
        {body}
      </p>
    </div>
  );
